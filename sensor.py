...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    account_ids = config.get(CONF_ACCOUNTS)

    if account_ids:
        _LOGGER.info('Adding sensors for account ids: ' +
                     ', '.join(account_ids))
    else:
        _LOGGER.info('Adding sensors for all discovered accounts')

    try:
        api_object = MESAPI(username, password)

        entities = []
        account_count = 0
        meter_count = 0
        for account_object in api_object.GetAccountsList(account_ids):
