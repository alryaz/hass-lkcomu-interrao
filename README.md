...
#### Счётчик
| Атрибут(ы) | Тип | Доступ | Назначение |
| ---------- | --- | ------ | ---------- |
| `meter_code` | `str` | Всегда | Уникальный номер счётчика
| `account_code` | `str` | Всегда | Уникальный номер связанного аккаунта
| `submit_period_start` | `str` (`date`) | При наличии полной<br>информации о периоде | Дата начала периода передачи показаний
| `submit_period_end` | `str` (`date`) | При наличии полной<br>информации о периоде | Дата окончания периода передачи показаний
| `install_date` | `str` (`date`) | При наличии даты установки | Дата установка счётчика
| `tariff_{ID}_name` | `str` / `None` | Всегда, если тариф _ID_ существует | Наименование тарифа _ID_, применяемого на счётчике
| `tariff_{ID}_cost` | `float` / `None` | Всегда, если тариф _ID_ существует | Стоимость за единицу тарифа _ID_, применяемого на счётчике
| `tariff_{ID}_description` | `str` / `None` | Всегда, если тариф _ID_ существует | Описание тарифа _ID_, применяемого на счётчике
| `tariff_{ID}_unit` | `str` / `None` | Всегда, если тариф _ID_ существует | Единица измерения тарифа _ID_, применяемого на счётчике
| `last_value_{ID}` | `str` / `None` | Всегда, если тариф _ID_ существует | Единица измерения тарифа _ID_, применяемого на счётчике
...
| - | - | - |
| `entity_id`<sup>\*</sup> | Идентификатор сенсора счётчика **(не совместимо с `meter_code`)**
| `meter_code`<sup>\*</sup> | Номер счётчика **(не совместимо с `entity_id`)**
...
- `success`: Если операция была выполнена успешно
- `entity_id`: Идентификатор сенсора счётчика
- `meter_code`: Номер счётчика
- `indications`: Нормализованный<sup>2</sup> список значений показаний из запроса
...
- `comment`: Текстовый комментарий, сообщаемый сервером ЛК, или текст внутренней ошибки
...
| `indications`<sup>\*</sup> | Список значений по показаниям (список длиной количества тарифов целевого счётчика из целых или дробных чисел)
| `ignore_period` | Не учитывать ограничения по периоду | `false`<br>учитывать ограничения
| `incremental` | Прибавлять указанные значения поверх последних переданных <sup>4</sup> | `false`<br>передавать показания без изменений
| `notification` | Показывать уведомление при успешном подсчёте начислений<sup>6</sup> | `false`<br>не показывать)

которая в случае успешного вызова отсылает событие `mosenergosbyt_push_result`, содержащее следующие данные:
- `success`: Если операция была выполнена успешно
- `entity_id`: Идентификатор сенсора счётчика
- `meter_code`: Номер счётчика
- `indications`: Нормализованный<sup>5</sup> список значений показаний из запроса
- `comment`: Текстовый комментарий, сообщаемый сервером ЛК, или текст внутренней ошибки

<sup>4</sup> Последние переданные показания включают в себя те, что отображаются под `submitted_value_t[1,2,3]`  
<sup>5</sup> Будут возвращены значения, полученные после прибавления последних предыдущих показаний
<sup>6</sup> Для уведомлений возможно использовать формат службы `persistent_notification.create`. Будут
             заменены все значения в тексте, которые имеют вид `{имя_переменной}` из списка переменных выше.
             **Это НЕ шаблоны Home Assistant!**
